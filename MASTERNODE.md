...
This is a complete guide to setting up a Masternode for Qureno Coin. The method employed is to use a cold Windows wallet together with a hot Ubuntu Linux VPS.  The reason is so that your coin will be safe in your Windows wallet and the VPS side will not contain any coins.

#### What you will need (recommended) ####
1. Download the latest windows Qureno wallet from here.
2. Get 10000 QRN coins (you can get 1 more coin to cover transaction fee)
...
- Next, get the Qureno auto install script by copying the entire command below:

```bash
wget https://raw.githubusercontent.com/qureno/qureno/master/contrib/masternode/install.sh
```
...
#### Next ####
- On the windows client (your PC or laptop), extract downloaded Qureno windows wallet
- Run the wallet (qureno-qt.exe)
- Wait for it to load
...
```bash
/opt/qureno-core/qurenod -daemon
```
