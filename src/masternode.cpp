...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if(GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if((out.nValue == MASTERNODE_COLLATERAL_AMOUNT * COIN  || out.nValue == MASTERNODE_OLD_COLLATERAL_AMOUNT * COIN)  && out.scriptPubKey == payee)  return true;
    }
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %s QRN, masternode=%s\n", MASTERNODE_COLLATERAL_AMOUNT, vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 10000 QRN tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 10000 QRN tx -> 1 confirmation
