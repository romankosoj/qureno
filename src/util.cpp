...
const char * const BITCOIN_CONF_FILENAME = "qureno.conf";
const char * const BITCOIN_PID_FILENAME = "qurenod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "qureno" is a composite category enabling all Qureno-related debug output
            if(ptrCategory->count(string("qureno"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Qureno
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Qureno
    // Mac: ~/Library/Application Support/Qureno
    // Unix: ~/.qureno
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qureno";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Qureno";
#else
    // Unix
    return pathRet / ".qureno";
#endif
#endif
}
...
/*
    if (!boost::filesystem::exists(GetConfigFile())){
        // Create empty qureno.conf if it does not excist
...
*/
    ...
    {
        // Don't overwrite existing settings so command line settings override qureno.conf
